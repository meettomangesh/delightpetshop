...
function ftc_testimonial_shortcode($atts){
	...
	if( $testimonials->have_posts() ){
		...
		?>
		<div class="vertical-testimonial loading">
	...
	function ftc_blogs_shortcode( $atts, $content = null){
		...
		if( $posts->have_posts() ):
			...
			$atts = compact('title', 'columns', 'categories', 'per_page', 'orderby', 'order'
				...
				,'is_slider', 'show_nav', 'auto_play', 'margin','is_grid', 'is_masonry', 'show_load_more','desksmall_items', 'tablet_items', 'tabletmini_items', 'mobile_items', 'mobilesmall_items');
				?>
				<div class="<?php echo esc_attr(implode(' ', $classes)); ?> ftc-shortcode"  data-atts="<?php echo htmlentities(json_encode($atts)); ?>" >
					...
					<?php endif; ?>
					<div class="meta-slider">
						<div class="blogs <?php if (!$is_masonry && !$is_grid){ ?> owl-carousel <?php } ?>">
						...
						<header class="entry-header">
							...
							<?php if( $show_author ): ?>
								<span class="vcard author">
									...
									<?php esc_html_e('','themeftc') ?><?php the_author_posts_link(); ?>
								</span>
							<?php endif; ?>
							...
							<?php if ( $categories_list && $smof_data['ftc_blog_categories'] ): ?>
								<div class="caftc-link">
									<span><?php esc_html_e('Categories: ', 'carna'); ?></span>
									<span class="cat-links"><?php echo trim($categories_list); ?></span>
								</div>
							<?php endif; ?>

							<?php if( $show_view ): ?>
								<span class="count-view"><?php esc_html_e('View: ','carna');?><?php echo get_post_views(get_the_ID()); ?></span>
							<?php endif; ?>
							<!-- Blog Title -->
							<?php if( $show_title ): ?>
...
//                var_dump($image_size);
	...
	if( !function_exists('ftc_products_category_tabs_shortcode') ){
		function ftc_products_category_tabs_shortcode($atts, $content){
			...
			$atts = compact( 'columns', 'rows', 'per_page', 'show_short_desc', 'active_tab'
				, 'is_slider', 'show_nav', 'auto_play');
