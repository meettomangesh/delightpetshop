<?php
/**
 * Carna functions and definitions
 *
 ...
 */
/**
 * Carna only works in WordPress 4.8 or later.
 */
...
function ftc_custom_sidebar_form() {
    ...
    <form action="<?php echo admin_url('widgets.php'); ?>" method="post" id="ftc-form-add-sidebar">
        <input type="text" name="sidebar_name" id="sidebar_name" placeholder="<?php esc_html_e('Custom Sidebar Name', 'peto') ?>" />
        <button class="button-primary" id="ftc-add-sidebar"><?php esc_html_e('Add Sidebar', 'peto') ?></button>
    </form>
    <?php
}
...
if( ! function_exists( 'peto_header_mobile_navigation' ) ) {
    function peto_header_mobile_navigation() {
        global $smof_data;
        ?>
...
$file_names = array('feedburner_subscription', 'products', 'blogs', 'blogs_tabs', 'recent_comments', 'product_categories','product_filter_by_color');
foreach ($file_names as $file) {
...
function ftc_setup() {
     ...
     * If you're building a theme based on Carna, use a find and replace
     * to change 'peto' to the name of your theme in all the template files.
     */
    load_theme_textdomain('peto');
    ...
    /* Translation */
    load_theme_textdomain('peto', get_template_directory() . '/languages');
    ...
    register_nav_menus(array(
        'primary' => esc_html__('Primary Navigation', 'peto'),
        'vertical' => esc_html__('Vertical Navigation', 'peto'),
    ));
    ...
    $starter_content = array(
        ...
        'nav_menus' => array(
            ...
            'social' => array(
                'name' => esc_html__('Social Links Menu', 'peto'),
                'items' => array(
                    'link_yelp',
                    'link_facebook',
                    'link_twitter',
    ...
    /**
     * Filters Carna array of starter content.
     *
     * @since Carna 1.1
     *
...
function ftc_content_width() {
    ...
    /**
     * Filter Carna content width of the theme.
     *
     * @since Carna 1.0
     *
...
function ftc_fonts_url() {
     ...
     */
    $dosis = esc_html__('on', 'peto');

    if ('off' !== $dosis) {
        $font_families = array();

        $font_families[] = 'Dosis:400,500,700,800';
...
function ftc_excerpt_more($link) {
    ...
    $link = sprintf('<p class="link-more"><a href="%1$s" class="more-link">%2$s</a></p>', esc_url(get_permalink(get_the_ID())),
            /* translators: %s: Name of current post */ sprintf(esc_html__('Continue reading<span class="screen-reader-text"> "%s"</span>', 'peto'), get_the_title(get_the_ID()))
    );
    return ' &hellip; ' . $link;
}
...
function ftc_scripts() {
    ...
    wp_enqueue_script('cookie', get_template_directory_uri().'/assets/js/jquery.cookie.min.js',
    ...
    wp_register_script( 'throttle', get_template_directory_uri() . '/assets/js/throttle.js' , array(), null, true );
    wp_enqueue_script( 'throttle' );

    wp_register_script( 'hoverIntent', get_template_directory_uri() . '/assets/js/hoverIntent.js' , array(), null, true );
    wp_enqueue_script( 'hoverIntent' );

    wp_register_script( 'greensock', get_template_directory_uri() . '/assets/js/greensock.js' , array(), null, true );
    wp_enqueue_script( 'greensock' );

    wp_register_script( 'waypoints', get_template_directory_uri() . '/assets/js/waypoints.js' , array(), null, true );
    wp_enqueue_script( 'waypoints' );

    wp_register_script( 'parallax', get_template_directory_uri() . '/assets/js/parallax.js' , array(), null, true );
    wp_enqueue_script( 'parallax' );

    wp_register_script( 'ytplayer', get_template_directory_uri() . '/assets/js/ytplayer.js' , array(), null, true );
...
wp_enqueue_script('threesixty', get_template_directory_uri() . '/assets/js/threesixty.min.js', array(), null, true);
    ...
    wp_register_script( 'prettyPhoto', get_template_directory_uri() . '/assets/js/prettyPhoto.js' , array(), null, true );
    wp_enqueue_script( 'prettyPhoto' );

    wp_register_script( 'isotope', get_template_directory_uri() . '/assets/js/isotope.js' , array(), null, true );
    ...
    if (has_nav_menu('top')) {
        ...
        $ftc_l10n['expand'] = esc_html__('Expand child menu', 'peto');
        $ftc_l10n['collapse'] = esc_html__('Collapse child menu', 'peto');
...
if ( ! function_exists( 'peto_popup_newsletter' ) ) {
    ...
    echo '</div>';
    echo '<span class="dont_show_popup"><input id="ftc_dont_show_again" type="checkbox"><label for="ftc_dont_show_again">' .esc_attr__('Don\'t show popup again', 'peto'). '</label></span>';
    echo '</div>';
}
}
...
if (!function_exists('ftc_get_search_form_by_category')) {
    ...
    function ftc_get_search_form_by_category() {
        ...
        if ($search_for_product) {
            ...
            $post_type = 'product';
            $placeholder_text = esc_html__('Search ...', 'peto');
        } else {
            $taxonomy = 'category';
            $post_type = 'post';
            $placeholder_text = esc_html__('Search', 'peto');
        ...
        $options = '<option value="">' . esc_html__('All categories', 'peto') . '</option>';
		...
		<form method="get" id="searchform' . $rand . '" action="' . esc_url(home_url('/')) . '">
		 ...
		 <div class="ftc_search_ajax">
			 ...
			 <input type="submit" title="' . esc_attr__('Search', 'peto') . '" id="searchsubmit' . $rand . '" value="' . esc_attr__('Search', 'peto') . '" />
    ...
    function ftc_ajax_search() {
        ...
        if ($results->have_posts()) {
            ...
            if (isset($results->post_count, $results->found_posts)) {
                $view_all_text = sprintf(esc_html__('View all %d results', 'peto'), $results->found_posts);
            ...
            $html = '<div class="eror-search"><span class="error">'.esc_html__('No item found.', 'peto').'</span></div';
            $return = array();
            $return['html'] = $html;
            $return['search_string'] = $search_string;
            die(json_encode($return));
...
if (!function_exists('ftc_breadcrumbs')) {
    ...
    function ftc_breadcrumbs() {
        ...
        if (!empty($front_id)) {
            ...
            $home = esc_html__('Home', 'peto');
        }
        $ar_title = array(
            'search' => esc_html__('Search results for ', 'peto')
            , '404' => esc_html__('Error 404', 'peto')
            , 'tagged' => esc_html__('Tagged ', 'peto')
            , 'author' => esc_html__('Articles posted by ', 'peto')
            , 'page' => esc_html__('Page', 'peto')
            , 'portfolio' => esc_html__('Portfolio', 'peto')
        );
            ...
            <div class="ftc-cookies-popup">
                <div class="ftc-cookies-inner">
                    ...
                    <div class="cookies-buttons">
                        <?php if ( $page_id ): ?>
                            <a href="<?php echo get_permalink( $page_id ); ?>" class="cookies-more-btn"><?php esc_html_e( 'More info' , 'peto' ); ?></a>
                        <?php endif ?>
                        <a href="#" class="cookies-accept-btn"><?php esc_html_e( 'Accept' , 'peto' ); ?></a>
                    </div>
                </div>
            </div>
        <?php
