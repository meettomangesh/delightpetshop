...
class Dokan_Category_Walker extends Walker {
    ...
    function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( $depth == 0 ) {
            $caret = $args['has_children'] ? ' <span class="caret-icon"><i class="fa fa-truck" aria-hidden="true"></i></span>' : '';
...
class Dokan_Category_Widget extends WP_Widget {
    ...
    public function __construct() {
        $widget_ops = array( 'classname' => 'dokan-category-menu', 'description' => __( 'Dokan product category menu', 'peto' ) );
    ...
    function form( $instance ) {
        $instance = wp_parse_args( (array) $instance, array(
            'title' => __( 'Product Category', 'peto' )
        ) );

        $title = $instance['title'];
        ?>
        <p>
            <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'peto' ); ?></label>
