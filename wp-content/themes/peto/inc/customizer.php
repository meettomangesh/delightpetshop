...
function ftc_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'colorscheme', array(
		'type'    => 'radio',
		'label'    => esc_html__( 'Color Scheme', 'peto' ),
		'choices'  => array(
			'light'  => esc_html__( 'Light', 'peto' ),
			'dark'   => esc_html__( 'Dark', 'peto' ),
			'custom' => esc_html__( 'Custom', 'peto' ),
		),
		'section'  => 'colors',
		'priority' => 5,
	) );
	...
	$wp_customize->add_section( 'theme_options', array(
		'title'    => esc_html__( 'Theme Options', 'peto' ),
	...
	$wp_customize->add_control( 'page_layout', array(
		...
		'type'        => 'radio',
		'description' => esc_html__( 'When the two column layout is assigned, the page title is in one column and content is in the other.', 'peto' ),
		'choices'     => array(
			'one-column' => esc_html__( 'One Column', 'peto' ),
			'two-column' => esc_html__( 'Two Column', 'peto' ),
		),
		'active_callback' => 'ftc_is_view_with_layout_option',
	) );

	/**
	 * Filter number of front page sections in Carna.
	 *
	 * @since Carna 1.0
	 *
	...
	for ( $i = 1; $i < ( 1 + $num_sections ); $i++ ) {
		...
		$wp_customize->add_control( 'panel_' . $i, array(
			...
			'label'          => sprintf( esc_html__( 'Front Page Section %d Content', 'peto' ), $i ),
			'description'    => ( 1 !== $i ? '' : esc_html__( 'Select pages to feature in each area from the dropdowns. Add an image to a section by setting a featured image in the page editor. Empty sections will not be displayed.', 'peto' ) ),
			'section'        => 'theme_options',
			'type'           => 'dropdown-pages',
			'allow_addition' => true,
