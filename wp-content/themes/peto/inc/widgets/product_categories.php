...
if(!class_exists('Ftc_Product_Categories_Widget')){
	class Ftc_Product_Categories_Widget extends WP_Widget {
		...
		function __construct() {
			$widgetOps = array('classname' => 'ftc-product-categories-widget', 'description' => esc_html__('Display Your Product Categories', 'peto'));
			parent::__construct('ftc_product_categories', esc_html__('FTC - Product Categories', 'peto'), $widgetOps);
		}
...
//							}
		...
		function form( $instance ) { 
			...
			<p>
                            ...
                            <label for="<?php echo esc_attr( $this->get_field_id('show_post_count')); ?>"><?php esc_html_e('Show post count', 'peto'); ?></label>
			</p>
			<p>
                            ...
                            <label for="<?php echo esc_attr( $this->get_field_id('show_sub_cat')); ?>"><?php esc_html_e('Show sub categories', 'peto'); ?></label>
			</p>
			<p>
                            ...
                            <label for="<?php echo esc_attr( $this->get_field_id('hide_empty')); ?>"><?php esc_html_e('Hide empty categories', 'peto'); ?></label>
			</p>
			<p>
				...
				<div class="categorydiv">
					<div class="tabs-panel">
						<ul class="categorychecklist">
							...
							<li>
								<label>
                                                                    <input type="checkbox" name="<?php echo esc_attr( $this->get_field_name('include_cat')); ?>[<?php $cat->term_id; ?>]" value="<?php echo $cat->term_id; ?>" <?php echo (in_array($cat->term_id,$instance['include_cat']))?'checked':''; ?> />
				...
				<span class="description"><?php esc_html_e("Don't select to show all", "peto"); ?></span>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id('orderby')); ?>"><?php esc_html_e('Order by', 'peto'); ?></label>
                                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id('orderby')); ?>" name="<?php echo esc_attr( $this->get_field_name('orderby')); ?>" >
					<option value="name" <?php selected($instance['orderby'], 'name'); ?> ><?php esc_html_e('Name', 'peto'); ?></option>
					<option value="slug" <?php selected($instance['orderby'], 'slug'); ?> ><?php esc_html_e('Slug', 'peto'); ?></option>
					<option value="count" <?php selected($instance['orderby'], 'count'); ?> ><?php esc_html_e('Number product', 'peto'); ?></option>
					<option value="rand" <?php selected($instance['orderby'], 'rand'); ?> ><?php esc_html_e('Random', 'peto'); ?></option>
					<option value="none" <?php selected($instance['orderby'], 'none'); ?> ><?php esc_html_e('None', 'peto'); ?></option>
				</select>
			</p>
			<p>
                            <label for="<?php echo esc_attr( $this->get_field_id('order')); ?>"><?php esc_html_e('Order', 'peto'); ?></label>
                            <select class="widefat" id="<?php echo esc_attr( $this->get_field_id('order')); ?>" name="<?php echo esc_attr( $this->get_field_name('order')); ?>" >
					<option value="asc" <?php selected($instance['order'], 'asc'); ?> ><?php esc_html_e('Ascending', 'peto'); ?></option>
					<option value="desc" <?php selected($instance['order'], 'desc'); ?> ><?php esc_html_e('Descending', 'peto'); ?></option>
				</select>
			</p>
		...
		function get_sub_categories( $category_id, $instance, $current_cat ){
			$args = array(
			   ...
			   ,'title_li'     => ''
			   ,'hide_empty'   => $instance['hide_empty']
			   ,'includes'	   => implode(',', $instance['include_cat'])
			);
