...
function ftc_custom_header_setup() {
	...
	/**
	 * Filter Carna custom-header support arguments.
	 *
	 * @since Carna 1.0
	 *
	 * @param array $args {
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'peto' ),
		),
	) );
}
...
function ftc_video_controls( $settings ) {
	$settings['l10n']['play'] = '<span class="screen-reader-text">' . esc_html__( 'Play background video', 'peto' ) . '</span>' . ftc_get_svg( array( 'icon' => 'play' ) );
	$settings['l10n']['pause'] = '<span class="screen-reader-text">' . esc_html__( 'Pause background video', 'peto' ) . '</span>' . ftc_get_svg( array( 'icon' => 'pause' ) );
	return $settings;
}
