...
if (!function_exists('ftc_tiny_account')) {
    ...
    function ftc_tiny_account() {
        ...
        <div class="ftc-account">
            <div class="ftc_login">
                <?php if (!$_user_logged): ?>
                    <a  class="login" href="<?php echo esc_url($login_url); ?>" title="<?php esc_html_e('Login', 'peto'); ?>"><span><?php esc_html_e('Login', 'peto'); ?></span></a>
                    / 
                    <a class="ftc_sign_up" href="<?php echo esc_url($register_url); ?>" title="<?php esc_html_e('Create New Account', 'peto'); ?>"><span><?php esc_html_e('Sign up', 'peto'); ?></span></a>
                <?php else: ?>
                    <a class="my-account" href="<?php echo esc_url($profile_url); ?>" title="<?php esc_html_e('My Account', 'peto'); ?>"><span><?php esc_html_e('My Account', 'peto'); ?></span></a> / 
                    <a class="log-out" href="<?php echo esc_url($logout_url); ?>" title="<?php esc_html_e('Logout', 'peto'); ?>"><span><?php esc_html_e('Logout', 'peto'); ?></span></a>
                <?php endif; ?>
            </div>
            <?php if (!$_user_logged): ?>
                <div class="ftc_account_form dropdown-container">
                    <form name="ftc-login-form" class="ftc-login-form" action="<?php echo esc_url(wp_login_url()); ?>" method="post">
                        ...
                        <p class="login-username">
                            <label><?php esc_html_e('Username', 'peto'); ?></label>
                            <input type="text" name="log" class="input" value="" size="20" autocomplete="off">
                        </p>
                        <p class="login-password">
                            <label><?php esc_html_e('Password', 'peto'); ?></label>
                            <input type="password" name="pwd" class="input" value="" size="20">
                        </p>

                        <p class="login-submit">
                            <input type="submit" name="wp-submit" class="button-secondary button" value="<?php esc_html_e('Login', 'peto'); ?>">
                            <input type="hidden" name="redirect_to" value="<?php echo esc_url($redirect_to); ?>">
                        </p>

                    </form>

                    <p class="ftc_forgot_pass"><a href="<?php echo esc_url(wp_lostpassword_url()); ?>" title="<?php esc_html_e('Forgot Your Password?', 'peto'); ?>"><?php esc_html_e('Forgot Your Password?', 'peto'); ?></a></p>
                </div>
            <?php endif; ?>
        </div>
    ...
    <?php if($smof_data['ftc_cart_layout'] == 'off-canvas'): ?>
        <div class="ftc-off-canvas-cart">
            <div class="off-canvas-cart-title">
                <div class="title"><?php esc_html_e('Shopping Cart', 'peto'); ?></div>
                <a href="#" class="close-cart"> <?php esc_html_e('Close', 'peto') ?></a>
            </div>
...
function ftc_tini_wishlist() {
    ...
    <a title="<?php esc_html_e('Wishlist', 'peto'); ?>" href="<?php echo esc_url($wishlist_page); ?>" class="tini-wishlist">
      <i class="fa fa-heart"></i>  
    <?php esc_html_e('Wishlist', 'peto'); ?> <span class="count-wish"><?php echo '(' . ($count > 0 ? zeroise($count, 1) : '0') . ')'; ?></span>
    </a>
